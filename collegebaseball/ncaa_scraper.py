...
def get_results(school, season):
    ...
    Returns: 
        ...
        innings_played, runs_scored, runs_allowed, 
    ...
    try: 
        ...
        return res.sort_values(by='date', ascending=True)
    except: 
...
def get_gbg_stats(school=None, player=None, season=None, variant='batting', advanced=True): 
    """
    ...
    Args: 
        ...
        season: season as (int, YYYY) or NCAA season_id (int)
        advanced: whether to include advanced stats (bool)
    ...
    Returns: 
        ...
        if advanced, also:
        PA, 1B, OBP, BA, SLG, OPS, ISO, PA/HR, K, BB
        BABIP, wOBA, wRAA, wRC
        ...
        if advanced, also: 
        IP-adj, 1B-A, OBP-against, BA-against, SLG-against
        OPS-against, K/PA, K/9, BB/PA, BB/9, BABIP-against
        FIP, ERA, WHIP, PA/HR, Pitches/PA
    ...
    if player:
        ...
        player_name = 'None'
        player_id = '-100' # gives team stats
        if type(school) == str: 
            school_name = school
    ...
    if variant == 'batting':
        ...
        if season >= 2016: 
            headers = ['date', 'field', 'season_id', 'opponent_id', 'opponent_name', \
                    'innings_played', 'runs_scored', 'runs_allowed', \
                    'run_difference', 'result', 'game_id', 'school_id', \
...
def get_career_stats(stats_player_seq, variant, advanced=True):
    """
...
def get_team_stats(school, season, variant, advanced=True):
    """
...
def lookup_seasons_played(stats_player_seq): 
    ...
    return int(row['first_season'].values[0]), int(row['last_season'].values[0])
...
def lookup_player_id(player_name, school):
    ...
    """
    df = _PLAYERS_HISTORY_LU_DF
    player_row = df.loc[df.name == player_name]
    if type(school) == str: 
        school_id = lookup_school_id(school)
    if len(player_row) == 1: 
        return int(player_row['stats_player_seq'].values[0])
    elif len(player_row) > 1:
        player_row = player_row.loc[school_id.isin(player_row.all_school_ids)]
        if len(player_row) == 1: 
            return int(player_row['stats_player_seq'].values[0])
        else:
...
def lookup_player_reverse(player_id, season):
    ...
    Args: 
        player_id (int): id of player to lookup
        school: either the ncaa_name of the school (str) of the player
    ...
    Examples: 
        lookup_player_reverse(2486499)
        >>> ('Jake Gelof', 'Virginia', 746)
    """
    df = _ROSTERS_DF
