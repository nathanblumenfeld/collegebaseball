...
def calculate_wrc(row):
    ...
    Linear weights calculated by Robert Fray
    """
    if row['PA'] > 0:
        season_weights = load_season_weights(row['season'])
...
def add_batting_metrics(df):
    ...
    try: 
        ...
        df.loc[:, 'wOBA'] = df.apply(calculate_woba, axis = 1)
        df.loc[:, 'wRAA'] = df.apply(calculate_wraa, axis = 1)
        df.loc[:, 'wRC'] = df.apply(calculate_wrc, axis = 1)
...
def calculate_fip(row):
    ...
    cFIP calculated by Robert Fray
    """
    if row['App'] > 0:
        season_weights = load_season_weights(row['season'])
        return round(((13 * row['HR-A'] 
                    + 3 * (row['BB']+row['HB'])
                    - 2 * row['SO'])/row['IP-adj'])+season_weights['cFIP'].values[0], 3)
    else: 
        print('no records found')
        return 0.00
